 ...
 *
 * Endpoints used:
 *   GET /api/v2/{dataset}/listings — closed sales + actives + pendings
 *
 * Auth: server-side token, sent as a Bearer header. We avoid the
 * query-string `access_token` form so it doesn't end up in access logs.
 */
...
export class BridgeProvider implements CompDataProvider {
  ...
  async resolveSubject(query: SubjectQuery): Promise<SubjectProperty | null> {
    // Try an exact UnparsedAddress match first; some MLSes prefer the
    // street-only form. Fall back to a contains match if we strike out.
    const filters = [
      `UnparsedAddress eq '${escapeOData(buildFullAddress(query))}'`,
      `UnparsedAddress eq '${escapeOData(query.address)}'`,
    ];
    for (const filter of filters) {
      const url = `${this.base}/${encodeURIComponent(this.config.dataset)}/listings?filter=${encodeURIComponent(
        filter
      )}&fields=${FIELDS}&limit=1`;
      const json = await this.get(url).catch(() => null);
      const first = pickArray<any>(json?.bundle)[0];
      if (first) return mapBridgeToSubject(first);
    }
    return null;
  }
  ...
  async pullComps(
    ...
    const soldFilter =
      `StandardStatus eq 'Closed' and CloseDate gt ${cutoff} and ` +
    ...
    const liveFilter =
      ...
      `StandardStatus eq 'Active Under Contract') and ` +
      `BedroomsTotal ge ${Math.max(1, subject.beds - 1)} and ` +
      `BedroomsTotal le ${subject.beds + 1} and ` +
      `LivingArea ge ${Math.round(subject.sqft * 0.8)} and ` +
      `LivingArea le ${Math.round(subject.sqft * 1.2)}`;
    ...
    for (const r of pickArray<any>(solds?.bundle)) {
      const rec = mapBridgeToComp(r, subject);
      if (rec) out.push(rec);
    }
...
function mapBridgeToSubject(r: any): SubjectProperty | null {
...
function mapBridgeToComp(r: any, subject: SubjectProperty): CompRecord | null {
  const beds = num(r?.BedroomsTotal);
  const baths = combinedBaths(r);
  const sqft = num(r?.LivingArea);
  ...
  const price =
    status === "sold"
      ? num(r?.ClosePrice) ?? num(r?.ListPrice)
      : num(r?.ListPrice) ?? num(r?.ClosePrice);
